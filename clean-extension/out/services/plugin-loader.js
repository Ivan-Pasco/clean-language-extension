...
class PluginLoader {
     ...
     */
    async loadPluginsFromWorkspace() {
        const errors = [];
     ...
     */
    async loadPlugin(name, workspacePath) {
        ...
        const config = vscode.workspace.getConfiguration('clean');
        const globalPath = config.get('plugins.globalPath', '~/.clean/plugins');
            ...
            if (kvMatch) {
                ...
                if (value.startsWith('[') && value.endsWith(']')) {
                    ...
                    if (currentSection === '' || currentSection === 'plugin') {
                        ...
                        if (key === 'blocks')
                            language.blocks = items;
                        else if (key === 'keywords')
                            language.keywords = items;
                        else if (key === 'types')
                            language.types = items;
                    }
                    continue;
                }
     ...
     */
    setupAppClnWatcher(context) {
        this.appClnWatcher = vscode.workspace.createFileSystemWatcher('**/app.cln', false, false, false);
    ...
    registerProviders(context) {
        // Completion provider
        this.completionProvider = vscode.languages.registerCompletionItemProvider([{ language: 'clean' }, { language: 'clean-html' }], {
        ...
        // Hover provider
        this.hoverProvider = vscode.languages.registerHoverProvider([{ language: 'clean' }, { language: 'clean-html' }], {
    ...
    getCompletionItems(document, position) {
        ...
        for (const plugin of this.loadedPlugins.values()) {
            ...
            for (const completion of lang.completions) {
                ...
                const item = new vscode.CompletionItem(completion.label, kindMap[completion.kind] || vscode.CompletionItemKind.Text);
