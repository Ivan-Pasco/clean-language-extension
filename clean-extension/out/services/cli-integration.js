...
class CLIIntegration {
    ...
    async createProject(name, template) {
        ...
        const choice = await vscode.window.showInformationMessage(`Project "${name}" created successfully!`, 'Open Project', 'Stay Here');
     ...
     */
    async dbReset() {
        const confirm = await vscode.window.showWarningMessage('This will delete all data in the database. Are you sure?', { modal: true }, 'Yes, Reset Database');
     ...
     */
    async generateEndpoint(routePath) {
        if (!routePath) {
     ...
     */
    async runCLICommand(args, message) {
