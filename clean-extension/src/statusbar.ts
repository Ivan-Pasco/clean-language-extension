...
import { ServerStatus } from './services/cli-integration';

export class CleanStatusBar {
	private cleanManager: CleanManagerIntegration;
	private versionStatusBarItem: vscode.StatusBarItem;
	private actionStatusBarItems: vscode.StatusBarItem[] = [];
	private serverStatusBarItem: vscode.StatusBarItem;
	private buildStatusBarItem: vscode.StatusBarItem;
	private pluginStatusBarItem: vscode.StatusBarItem;

	constructor(cleanManager: CleanManagerIntegration) {
	...
	async initialize(context: vscode.ExtensionContext): Promise<void> {
		...
		const editorChangeDisposable = vscode.window.onDidChangeActiveTextEditor(
			this.updateActionButtonsVisibility.bind(this)
		);

		context.subscriptions.push(
			this.versionStatusBarItem,
			this.serverStatusBarItem,
			this.buildStatusBarItem,
			this.pluginStatusBarItem,
	 ...
	 */
	updateServerStatus(status: ServerStatus, port?: number): void {
		switch (status) {
			case 'stopped':
				this.serverStatusBarItem.text = '$(circle-outline) Server';
	 ...
	 */
	updatePluginCount(count: number, pluginNames?: string[]): void {
		if (count === 0) {
			this.pluginStatusBarItem.hide();
			return;
		}
	 ...
	 */
	showTemporaryStatus(message: string, durationMs: number = 3000): void {
