...
export interface CleenDetectionResult {
	found: boolean;
	path?: string;
	version?: string;
	method?: string;
...
export class CleenDetector {
	private cachedResult?: CleenDetectionResult;
	 ...
	 */
	async detectCleenPath(configuredPath?: string): Promise<CleenDetectionResult> {
		// Check cache first
	 ...
	 */
	private getSearchPaths(): Array<{ path: string; method: string }> {
		...
		if (isWindows) {
			paths.push(
				{ path: path.join('C:', 'Program Files', 'cleen', cleenBinary), method: 'windows-program-files' },
				{ path: path.join(homeDir, 'AppData', 'Local', 'Programs', 'cleen', cleenBinary), method: 'windows-appdata' }
			);
		}
	 ...
	 */
	private async tryPath(cleenPath: string, method: string): Promise<CleenDetectionResult> {
		try {
	 ...
	 */
	private async tryWhichCommand(): Promise<CleenDetectionResult> {
		try {
	 ...
	 */
	private cacheResult(result: CleenDetectionResult): CleenDetectionResult {
	 ...
	 */
	async getActiveVersion(cleenPath: string): Promise<string | undefined> {
		try {
			const homeDir = os.homedir();
	...
	async runDiagnostics(): Promise<string> {
		...
		const relevantPaths = pathParts.filter(p =>
			p.includes('cargo') ||
			p.includes('cleen') ||
			p.includes('.local')
		);
